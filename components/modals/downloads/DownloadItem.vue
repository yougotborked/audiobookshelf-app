<template>
  <div class="flex items-center justify-center">
    <img v-if="download.cover" :src="(download.cover as string)" class="w-10 h-16 object-contain" />
    <img v-else src="/book_placeholder.jpg" class="w-10 h-16 object-contain" />
    <div class="pl-2 w-2/3">
      <p class="font-normal truncate text-sm">{{ (download.audiobook as any)?.book?.title }}</p>
      <p class="font-normal truncate text-xs text-gray-400">{{ (download.audiobook as any)?.book?.author }}</p>
      <p class="font-normal truncate text-xs text-gray-400">{{ utils.bytesPretty(download.size as number) }}</p>
    </div>
    <div class="flex-grow" />
