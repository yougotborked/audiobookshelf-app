<template>
  <div class="w-full h-full relative">
    ...
    <div class="fixed left-0 h-8 w-full bg-md-surface-1 px-4 flex items-center text-md-on-surface-variant" :style="{ bottom: isPlayerOpen ? '120px' : '0px' }">
      <div class="flex-grow" />
  ...
  methods: {
    async updateProgress() {
      ...
      if (this.serverLibraryItemId) {
        this.nativeHttp.patch(`/api/me/progress/${this.serverLibraryItemId}`, payload, { connectTimeout: 10000 }).catch((error) => {
